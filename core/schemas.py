...
class Phase2Triple(BaseTriple):
    ...
    semantic: str = Field(
        ..., description="Semantic definition output from Semantic Node", min_length=10
    )
...
class Phase3Triple(BaseTriple):
    ...
    tension: str = Field(
        ..., description="Tension analysis output from Tension Validator", min_length=10
    )
...
class ReformulatedQuestion(BaseModel):
    ...
    question: str = Field(
        ..., description="The reformulated, bias-free question", min_length=5
    )
    ...
    bias_removed: List[str] = Field(
        default_factory=list, description="List of bias elements identified and removed"
    )
...
class SynthesisOutput(BaseModel):
    ...
    raw_response: str = Field(
        ..., description="Raw LLM response from Layer 4 synthesis", min_length=1
    )
...
class ValidationResult(BaseModel):
    ...
    score: float = Field(
        ..., description="Validation score (0.0 to 1.0)", ge=0.0, le=1.0
    )
    assessment: str = Field(
        ..., description="Detailed validation assessment", min_length=10
    )
    confidence: float = Field(
        ..., description="Confidence in the validation result", ge=0.0, le=1.0
    )
    issues: List[str] = Field(
        default_factory=list, description="List of identified issues or concerns"
    )
...
class NetworkResponse(BaseModel):
    ...
    synthesis: Optional[SynthesisOutput] = Field(
        None, description="Final synthesis output if successful"
    )
    ...
    phase_timings: Dict[str, float] = Field(
        default_factory=dict, description="Processing time for each phase"
    )
