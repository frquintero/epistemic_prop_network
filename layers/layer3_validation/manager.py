...
class Layer3ValidationManager:
    ...
    def __init__(self, llm_configs: Optional[Dict[str, LLMConfig]] = None):
        """Initialize the validation manager with all Layer 3 validators.
        ...
        self.tension_validator = TensionValidator(
            llm_config=self.llm_configs.get("tension_validator")
        )
    ...
    async def process(self, phase2_triple: Phase2Triple) -> Phase3Triple:
        ...
        Returns:
            Phase3Triple containing validation results from all validators
        """
        logger.info(
            "Starting Layer 3 Validation | semantic_len=%d | genealogical_len=%d | teleological_len=%d",
            len(phase2_triple.semantic),
            len(phase2_triple.genealogical),
            len(phase2_triple.teleological),
        )
        ...
        tension_task = self.tension_validator.process(phase2_triple)

        try:
            # Wait for all validations to complete
            results = await asyncio.gather(
                coherence_task, pragmatic_task, tension_task, return_exceptions=True
            )

            # Handle results and exceptions
            (coherence_result, pragmatic_result, tension_result) = (
            ...
            logger.info(
                "Layer 3 Validation completed | processing_time=%.2fs | coherence_len=%d | pragmatic_len=%d | tension_len=%d",
                processing_time,
                len(coherence_result),
                len(pragmatic_result),
                len(tension_result),
            )
            ...
            return Phase3Triple(
                ...
                tension=self.tension_validator._get_fallback_response(),
            )
    ...
    def _handle_parallel_results(
        ...
        Returns:
            Tuple of (coherence_result, pragmatic_result, tension_result)
        """
        coherence_result = results[0]
        pragmatic_result = results[1]
        tension_result = results[2]

        # Handle coherence result
        if isinstance(coherence_result, Exception):
            logger.warning(
                "Coherence validation failed with exception | error=%s",
                str(coherence_result),
            )
        ...
        if isinstance(pragmatic_result, Exception):
            logger.warning(
                "Pragmatic validation failed with exception | error=%s",
                str(pragmatic_result),
            )
            pragmatic_result = self.pragmatic_validator._get_fallback_response()

        # Handle tension result
        if isinstance(tension_result, Exception):
            logger.warning(
                "Tension validation failed with exception | error=%s",
                str(tension_result),
            )
            tension_result = self.tension_validator._get_fallback_response()

        return coherence_result, pragmatic_result, tension_result
